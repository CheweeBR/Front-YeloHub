...
function StatusTransition({ pedido, onAtualizar }: StatusTransitionProps) {
  ...
  if (statusTerminal.includes(pedido.status)) {
    return (
      <div className="border border-zinc-800 bg-zinc-900 px-5 py-4">
        <p className="text-zinc-600 text-[10px] font-mono uppercase tracking-widest mb-2">Status do pedido</p>
  ...
  return (
    <div className="border border-zinc-800 bg-zinc-900 px-5 py-4">
      <p className="text-zinc-600 text-[10px] font-mono uppercase tracking-widest mb-3">Alterar status</p>
        ...
        {proximos.map((status) => {
          ...
          if (cfg.destructive) {
            return (
              <div key={status}>
                {!esteConfirmando ? (
                  <button onClick={() => setConfirmando(status)} disabled={atualizando || confirmando !== null}
                    className={`w-full sm:w-auto px-4 py-2.5 text-xs font-mono uppercase tracking-widest transition-colors duration-150 disabled:opacity-30 disabled:cursor-not-allowed ${cfg.style}`}>
                    {cfg.label}
                  </button>
                ) : (
                  <div className="flex items-center gap-2 flex-wrap">
                    <span className="text-zinc-400 text-xs font-mono">Confirmar?</span>
                    <button onClick={() => handleConfirmar(status)} disabled={atualizando}
                      className="px-4 py-2 bg-red-600 text-white text-xs font-mono uppercase tracking-widest hover:bg-red-500 disabled:opacity-50 transition-colors duration-150 flex items-center gap-1.5">
                      {atualizando && (
                    ...
                    <button onClick={() => setConfirmando(null)} disabled={atualizando}
          ...
          return (
            <button key={status} onClick={() => handleConfirmar(status)}
              disabled={atualizando || confirmando !== null}
              className={`w-full sm:w-auto px-4 py-2.5 text-xs font-mono uppercase tracking-widest transition-colors duration-150 disabled:opacity-30 disabled:cursor-not-allowed flex items-center gap-1.5 ${cfg.style}`}>
              {atualizando && (
                <svg className="w-3 h-3 animate-spin" fill="none" viewBox="0 0 24 24">
                  <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
                  <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8z" />
...
interface PedidoDetalheProps {
  ...
  exportButton?: ReactNode   // ← slot para o botão de PDF
}

export function PedidoDetalhe({ pedido, user, onVoltar, onAtualizarStatus, exportButton }: PedidoDetalheProps) {
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 py-6 space-y-5">
      ...
      <button onClick={onVoltar}
        ...
        <svg className="w-3.5 h-3.5 translate-x-0 group-hover:-translate-x-0.5 transition-transform duration-150"
      ...
      <div className="border border-zinc-800 bg-zinc-900 px-5 py-5">
        <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
          <div>
            ...
            <h2 className="text-white text-3xl uppercase leading-none"
        ...
        {user.role !== 'cliente' && (
          <div className="border border-zinc-800 bg-zinc-900 px-5 py-4">
            ...
            {pedido.clienteDocument && (
              <p className="text-zinc-500 text-xs font-mono mt-1">{pedido.clienteDocument}</p>
            )}
          </div>
        )}
        {user.role !== 'vendedor' && pedido.vendedorNome && (
          <div className="border border-zinc-800 bg-zinc-900 px-5 py-4">
            ...
            {pedido.vendedorDocument && (
              <p className="text-zinc-500 text-xs font-mono mt-1">{pedido.vendedorDocument}</p>
            )}
          </div>
        )}
      </div>
      ...
      <div className="border border-zinc-800 bg-zinc-900">
        ...
        {/* Header da seção de itens — com o botão de exportar PDF aqui */}
        <div className="px-5 py-3.5 border-b border-zinc-800 flex items-center justify-between">
          ...
          {/* Slot do botão de exportar — injetado de fora */}
          {exportButton}
        </div>
