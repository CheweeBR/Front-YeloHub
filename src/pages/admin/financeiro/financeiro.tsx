...
export default function FinanceiroPage() {
  ...
  const fin = useMemo(
    () => (user ? getFinanceiro(user.empresaId) : null),
    [user]
  )

  if (!user || !fin) return null
  ...
  const cards: DashboardCardData[] = [
    { key: 'faturado', label: 'Faturado', route: '/pedidos?status=concluido', value: fmt(fin.faturado), description: pluralPedido(fin.pedidosFaturados) + ' · concluídos e pagos', svg: SvgFaturado },
    { key: 'aguardando_pagamento', label: 'Aguardando pagamento', route: '/pedidos?status=pagamento_pendente', value: fmt(fin.aguardandoPagamento), description: pluralPedido(fin.pedidosPendentes) + ' · entregues, pagamento pendente', svg: SvgAguardando },
    { key: 'em_aberto', label: 'Em aberto', route: '/pedidos', value: fmt(fin.emAberto), description: pluralPedido(fin.pedidosEmAberto) + ' · em aprovação, preparo ou entrega', svg: SvgEmAberto },
  ]
